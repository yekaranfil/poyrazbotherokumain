...
module.exports = {
  name: "bump",
  description: "Bir şarkıyı kuyruğun önüne taşır.",
  usage: "",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!args[0])
      return client.sendTime(message.channel, "❌ | **Geçersiz argümanlar.**");

    // kontrol et if (args[0] - 1) geçerli bir dizin olmalı
    ...
    // Diziden kaldır ve diziyi değiştir
    const track = player.queue[trackNum];
    player.queue.splice(trackNum, 1);
    player.queue.unshift(track);
    client.sendTime(
      message.channel,
      "✅ | **" + track.title + " ** sıranın önüne taşındı."
    );
  },
